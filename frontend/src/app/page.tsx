...
import { Activity, Thermometer, Droplets, Wind, ClipboardList, Database, Zap } from "lucide-react";
  ...
  if (isLoadingDemographics || !selectedId) {
    return (
      <div className="h-screen w-full flex flex-col items-center justify-center bg-white gap-4">
         ...
         <span className="text-xs font-black text-slate-400 uppercase tracking-[0.4em] animate-pulse">Initializing Relational Database</span>
      </div>
    );
  }
      ...
      <div className="flex-1 overflow-y-auto px-12 py-10 space-y-10">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xs font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Clinic AI Engine</h2>
          ...
          <div className="flex items-center gap-2 px-4 py-2 bg-white rounded-2xl border border-slate-100 shadow-sm">
            ...
            <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Supabase Node: Active</span>
          </div>
        </motion.div>
        ...
        <motion.section 
          ...
          className="bg-white rounded-[3rem] border border-slate-100/50 shadow-2xl shadow-slate-200/20 overflow-hidden"
        >
          <div className="grid grid-cols-1 lg:grid-cols-3">
            {/* Case Identifiers */}
            <div className="lg:col-span-1 p-10 bg-slate-50/50 border-r border-slate-100">
              <div className="flex items-center gap-3 mb-8">
                <div className="w-8 h-8 rounded-lg bg-slate-900 text-white flex items-center justify-center">
                  <Database size={16} />
                </div>
                <h3 className="text-sm font-black text-slate-800 uppercase tracking-tight">Clinical Metadata</h3>
              </div>
              ...
              <div className="space-y-6">
                {[
                  ...
                  <div key={idx} className="flex justify-between items-center group">
                    ...
                    <span className="text-sm font-black text-slate-800 font-mono tracking-tighter">{item.val}</span>
                  </div>
                ))}
              </div>

              <div className="mt-10 p-6 rounded-[2rem] bg-blue-600 text-white shadow-xl shadow-blue-200">
            ...
            <div className="lg:col-span-2 p-10 relative">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  ...
                  <div key={idx} className="p-5 rounded-3xl border border-slate-100 hover:border-blue-100 hover:bg-blue-50/20 transition-all group">
                    <div className="flex justify-between items-center mb-1">
                      <h4 className="text-xs font-black text-slate-800 uppercase">{protocol.title}</h4>
                      <span className={cn(
                        "text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full",
                        protocol.status === "Critical" ? "bg-rose-50 text-rose-600" : "bg-slate-100 text-slate-500"
                      )}>{protocol.status}</span>
                    </div>
                    <p className="text-[10px] font-bold text-slate-400 group-hover:text-slate-500 leading-relaxed uppercase tracking-widest">
                      {protocol.desc}
                    </p>
                  </div>
                ))}
